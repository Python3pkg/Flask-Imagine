...
class TestUpdateFilterSet(TestCase):
    def test_wrong_parameters(self):
        with self.assertRaises(TypeError):
            self.imagine.update_filter_set()

        with self.assertRaises(ValueError):
            self.imagine.update_filter_set({}, [])

        with self.assertRaises(ValueError):
            self.imagine.update_filter_set('test_set', '')

        with self.assertRaises(ValueError):
            self.imagine.update_filter_set('test_set', [])

        with self.assertRaises(ValueError):
            self.imagine.update_filter_set('test_set', ['asd'])

        with self.assertRaises(ValueError):
            self.imagine.update_filter_set('test_set', [ThumbnailFilter(size=[100, 100], mode='inset')], '')

    def test_duplicate_filter_name(self):
        thumbnail_filter = ThumbnailFilter(size=[100, 100], mode='inset')
        self.imagine.update_filter_set('test_set', [thumbnail_filter, ])

        self.imagine.update_filter_set('test_set', [thumbnail_filter, ])

    def test_success_filter_update_with_cache(self):
        self.imagine.update_filter_set('test_dynamic_set_cached', [ThumbnailFilter(size=[100, 100], mode='inset')])
    ...
    def test_success_filter_update_without_cache(self):
        self.imagine.update_filter_set('test_dynamic_set', [ThumbnailFilter(size=[100, 100], mode='inset')], cached=False)
